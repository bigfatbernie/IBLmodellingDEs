...
\begin{goals}
	...
	\begin{itemize}
		...
		\begin{tikzpicture}
			...
			\draw[samples=20, ultra thick,domain=-1.25:1.25,smooth,variable=\x,blue] plot ({\x+1.5},{\x*\x*\x});
			\draw (3,1) node[right,blue] {\Huge ?};
		\end{tikzpicture}
	\end{itemize}
\end{goals}
\end{annotation}
...
\begin{tikzpicture}
  ...
  \draw[thick,-{\seta}] (0,-2) -- (0,2) node[left] {\small $y$};
  \draw[samples=40, ultra thick,domain=-4:4,smooth,variable=\x,blue] plot ({\x},{sin(1.5*deg(\x))/2-\x/4}) node[below] {$y_3$};
  \draw[samples=40, ultra thick,domain=0.075:4,smooth,variable=\x,gray] plot ({\x},{-1/((\x^(0.33))+\x)}) node[below] {$y_2$};
  \draw[samples=40, ultra thick,domain=-2.35:3,smooth,variable=\x,green!50!black] plot ({\x-1.5},{\x*\x*\x/8-\x*\x/3+1.6}) node[right] {$y_1$};
  \draw[dashed] (1,-2) -- (1,2);
  \draw (1,0) node[below] {$t_0$};
  \draw (1,-0.5) node {\tikzcircle[gray,fill=gray!50!white]{2pt}};
  \draw (1,{(2.5^3)/8-(2.5^2)/3+1.6}) node {\tikzcircle[green!50!black,fill=green]{2pt}};
  \draw (1,{sin(1.5*deg(1))/2-1/4}) node {\tikzcircle[blue,fill=cyan!50!white]{2pt}};
\end{tikzpicture}
...
\begin{parts}
	\item Can \textbf{\color{green} green $y1$} and \textbf{\color{blue} blue $y3$} be two solutions of the same differential equation above with two different initial conditions? Why?
	...
	\item Can \textbf{\color{blue} blue $y3$} and \textbf{\color{gray} gray $y2$} be two solutions of the same differential equation above with two different initial conditions? Why?
